...
function getTextMessage()
    ...
    if messageWord ~= previousMessageWord then
        local highByte = bit32.rshift(messageWord, 8)
        highByte = bit32.band(highByte, 127)
        local lowByte = bit32.rshift(messageWord, 1)
		lowByte = bit32.band(lowByte, 127)
...
function getDirectionToVehicle()
	...
	--directionToVehicle = 90
	return directionToVehicle
end

function getDirectionToPilot()
	local directionToPilot = -1
	local pilotlat = getValue("pilot-latitude")
	local pilotlon = getValue("pilot-longitude")
	local curlat = getValue("latitude")
	local curlon = getValue("longitude")
	...
	if pilotlat~=0 and curlat~=0 and pilotlon~=0 and curlon~=0 then
        directionToPilot = getDirectionFromTo(curlat, curlon, pilotlat, pilotlon)
	end
	return directionToPilot
end
...
local function drawTemperatureMotor(x,y)
	local totalCurrent = getValue("temp1")
    lcd.drawNumber(x, y, totalCurrent, MIDSIZE)
    lcd.drawText(lcd.getLastPos(), y+5, "C motor", SMLSIZE)
end

local function drawTemperatureEsc(x,y)
	local totalCurrent = getValue("temp2")
    lcd.drawNumber(x, y, totalCurrent, MIDSIZE)
    lcd.drawText(lcd.getLastPos(), y+5, "C esc", SMLSIZE)
end
...
local function drawSpeed(x,y)
    ...
    lcd.drawNumber(x + 47, y, speed, MIDSIZE)
...
local function drawAltitude(x, y)
    ...
    lcd.drawNumber(x + 46, y, altitude, MIDSIZE)
    local t = lcd.getLastPos() + 1
...
local function drawDistance(x, y)
    ...
    lcd.drawNumber(x + 46, y, distance, MIDSIZE)
    local t = lcd.getLastPos() + 1
    lcd.drawText(t, y + 5, "m", SMLSIZE)
end
...
local function getRelativeGroundDirection()
    ...
    if groundDirection >= 0 then
       ...
       if headingToVehicle >= 0 then
            local relativeHeading = groundDirection - headingToVehicle - 180
...
local function drawToHomeHud(x, y)
    local arrowSide = 5
    local arrowTail = 5
    local arrowLength = 16
    local arrowSideAngle = 120
    --local headingHudInnerRadius = 16
    local headingHudOuterRadius = 15
	local vehicleDirectionToHome = -1
    --ocal arming_state = getValue("accy")
	vehicleDirectionToHome = getRelativeGroundDirection()
    if vehicleDirectionToHome  == -1 then
      vehicleDirectionToHome = 0
    end

		vehicleDirectionToHome = vehicleDirectionToHome

		local xTail, yTail = getXYAtAngle(x, y, vehicleDirectionToHome - 180, arrowTail)
		local xLeft, yLeft = getXYAtAngle(xTail, yTail, vehicleDirectionToHome-arrowSideAngle, arrowSide)
		local xRight, yRight = getXYAtAngle(xTail, yTail, vehicleDirectionToHome+arrowSideAngle, arrowSide)
		local xNose, yNose = getXYAtAngle(xTail, yTail, vehicleDirectionToHome, arrowLength)
		lcd.drawLine(xTail, yTail, xLeft, yLeft, SOLID, FORCE)
		lcd.drawLine(xLeft, yLeft, xNose, yNose, SOLID, FORCE)
		lcd.drawLine(xTail, yTail, xRight, yRight, SOLID, FORCE)
		lcd.drawLine(xRight, yRight, xNose, yNose, SOLID, FORCE)

		 lcd.drawNumber(x + 20, y -10, vehicleDirectionToHome, INVERS)
...
local function run(event)
	...
	drawTemperatureMotor(60,12)
	drawTemperatureEsc(60,26)

    drawSats(72, 40)
    drawHdop(130, 40)

    drawToHomeHud(120, 26)
